...
public class LoginController implements CommunityConstant {
    ...
    public String register(Model model, User user) {
        ...
        if (map == null || map.isEmpty()) {
            model.addAttribute("msg", "注册成功，我们已经向您的邮箱发送了激活链接，请尽快激活哦~");
    ...
    public String activation(Model model, @PathVariable("userId") int userId, @PathVariable("code") String code) {
        ...
        if (result == ACTIVATION_SUCCESS) {
            model.addAttribute("msg", "激活成功，您的账号可以开始使用啦！");
            model.addAttribute("target", "/login");
        } else if (result == ACTIVATION_REPEAT) {
            model.addAttribute("msg", "无效操作，该账号已经激活过啦！");
            model.addAttribute("target", "/index");
        } else {
    ...
    public void getKaptcha(HttpServletResponse response /*HttpSession session*/) {
        ...
        try {
            OutputStream os = response.getOutputStream(); // 不需要手动关闭，由spring MVC管理
    ...
    public String logout(@CookieValue("ticket") String ticket) {
        ...
        return "redirect:/login"; // 默认为get方式的
    }
}
